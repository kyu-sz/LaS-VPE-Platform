...
/**
  ...
  */
class DirectKafkaInputDStreamForVPE[K, V](
  ...
  protected var currentOffsets: Map[TopicPartition, Long] = Map[TopicPartition, Long]()

  @transient private var kc: Consumer[K, V] = _
  ...
  protected def getBrokers: ju.HashMap[TopicPartition, String] = {
    ...
    while (assignments.hasNext) {
      ...
      logDebug("Broker for " + tp + " is " + hosts.get(tp))
  ...
  protected[streaming] def maxMessagesPerPartition(
    ...
    val effectiveRateLimitPerPartition = estimatedRateLimit.filter(_ > 0) match {
      ...
      case None => offsets.map { case (tp, _) => tp -> maxRateLimitPerPartition }
    }
  ...
  protected def clamp(
    ...
    logDebug("Clamping latest offsets: " + offsets.mkString("\n"))
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V]] = {
    ...
    logDebug("Clamped until offsets: " + untilOffsets.mkString("\n"))
